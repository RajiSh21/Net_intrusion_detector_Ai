...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:5000/api/stats")
      ...
      .then((data) => {
        if (!data.error) {
           setThreatsFound(data.threatsFound);
        }
      })
    ...
    socket.on("packet_update", (data) => {
      // Map backend model classes to frontend UI labels
      let uiVerdict = data.verdict;
      if (uiVerdict === "Normal") uiVerdict = "Benign";
      if (uiVerdict === "Dos/DDos") uiVerdict = "DoS / DDoS";
      if (uiVerdict === "PortScan") uiVerdict = "Port Scan";
      if (uiVerdict === "Botnet ARES") uiVerdict = "Botnet C2";
      if (uiVerdict === "Infiltration") uiVerdict = "Web Attack";

      // Reconstruct scores object to mock full probabilities for UI display
      const scores = { [uiVerdict]: data.confidence };
      const rest = 1 - data.confidence;
      const others = LABELS.filter((l) => l !== uiVerdict);
      others.forEach((l, i) => {
        scores[l] = i === others.length - 1 ? rest : rest * Math.random() * 0.6;
      });
      ...
      const p = {
        ...
        proto: data.proto,
        length: data.length,
        flags: FLAG_SETS[Math.floor(Math.random() * FLAG_SETS.length)], // mock flags
        ttl: [64, 128, 255][Math.floor(Math.random() * 3)], // mock ttl
        entropy: (Math.random() * 8).toFixed(2), // mock entropy
        latencyMs: (data.latencyMs || (0.4 + Math.random() * 2.1)).toFixed(2),
        verdict: uiVerdict,
        scores,
      };
  ...
  return (
    <div className="pc-root">
      ...
      {/* Chart + Breakdown + Model Info */}
      <div className="grid-lower" style={{ gridTemplateColumns: "1fr 1fr 1fr" }}>
        <div className="panel">
          <div className="panel-head">
        ...
        <div className="panel">
          <div className="panel-head">
            <span className="panel-title"><Cpu size={13} /> AI Model Telemetry</span>
            <span className="subtle">live</span>
          </div>
          {modelInfo ? (
            <div className="field-grid" style={{ gridTemplateColumns: "1fr", gap: "10px" }}>
              <div className="field"><span className="field-k">Engine</span><span className="field-v">{modelInfo.algorithm}</span></div>
              <div className="field"><span className="field-k">Accuracy</span><span className="field-v">{modelInfo.accuracy}</span></div>
              <div className="field"><span className="field-k">Training Samples</span><span className="field-v">{modelInfo.trainingSamples.toLocaleString()}</span></div>
              <div className="field"><span className="field-k">GAN Augmentation</span><span className="field-v" style={{ color: "var(--cyan)" }}>Active</span></div>
              <div className="field"><span className="field-k">Total Historical Threats</span><span className="field-v warn-val">{threatsFound.toLocaleString()}</span></div>
            </div>
          ) : (
            <div className="subtle" style={{ padding: "14px 10px" }}>Loading model data...</div>
          )}
        </div>
      </div>
