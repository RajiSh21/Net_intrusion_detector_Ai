...
# ============================================================
class AlertLogger:
    """Logs intrusion alerts to SQLite database and optional CSV."""

    def __init__(self, db_path: str = ALERT_DB_PATH, csv_path: Optional[str] = None,
            ...
            try:
                ...
                cursor.execute('''
                    INSERT INTO alerts (timestamp, src_ip, dst_ip, src_port, dst_port,
                                        protocol, prediction, confidence, num_packets,
                                        flow_duration_us, total_bytes, features_json)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    timestamp, src_ip, dst_ip, src_port, dst_port,
                    protocol, prediction, confidence, num_packets,
                    flow_duration, total_bytes, json.dumps(features.to_list())
                ))
                conn.commit()
                conn.close()
